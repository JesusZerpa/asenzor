// Transcrypt'ed from Python, 2021-01-17 03:47:21
...
export var ImageSelect =  __class__ ('ImageSelect', [VuePy], {
	...
	get data () {return __get__ (this, function (self) {
		var value = self.vue ['$root'].get_content (self.vue ['$attrs'] ['name']);
		if (value) {
			if (value) {
		...
		return dict ({'image': image, 'name': self.vue ['$attrs'] ['name']});
	});},
	get preview () {return __get__ (this, function (self, result) {
		self.vue.image = result [0];
		self.vue ['$root'].update_content (dict ([[self.vue ['$attrs'] ['name'], self.vue.image.src]]));
	});},
	get remove_img () {return __get__ (this, function (self) {
		self.vue.image = null;
		self.vue ['$root'].update_content (dict ([[self.vue ['$attrs'] ['name'], null]]));
	});},
